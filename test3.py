...
# ✅ sumw_txt 디렉토리 (너가 말한 위치)
SUMW_TXT_DIR = os.path.join(DATA_BASE, "sumw_txt")  # = "Data/sumw_txt"
...
# ============================================================
# 6) sumw_txt 로드 (여기가 핵심 변경)
# ============================================================
def parse_sumw_txt_file(txt_path: str):
    """
    sumw_txt/*.txt 포맷:
      source xxx.txt
      mode NEW/UPDATE
      ...
      nBad <int>
    """
    info = {"sumw": None, "nFiles": None, "nOk": None, "nBad": None, "warning": None}
    with open(txt_path, "r") as f:
        for line in f:
            ...
            key = toks[0]
            if key == "sumw":
                info["sumw"] = float(toks[1])
            elif key == "nFiles":
                info["nFiles"] = int(toks[1])
            elif key == "nOk":
                info["nOk"] = int(toks[1])
            elif key == "nBad":
                info["nBad"] = int(toks[1])
            elif key == "WARNING":
...
def load_sumw_maps(sumw_dir: str):
    """
    returns:
      sumw_map: { "DY2E_2J_50_MC_2024.root": sumw, ... }
      meta_map: { same key -> dict(nBad,nOk,...) }
    """
    if not os.path.isdir(sumw_dir):
        raise RuntimeError(f"[FATAL] sumw_txt dir not found: {sumw_dir}")

    sumw_map = {}
    meta_map = {}

    for fn in os.listdir(sumw_dir):
        if not fn.endswith(".txt"):
            continue
        if fn == "_badlists":
            continue
        txt_path = os.path.join(sumw_dir, fn)
        if not os.path.isfile(txt_path):
            continue

        info = parse_sumw_txt_file(txt_path)
        base = fn[:-4]          # remove .txt
        root_name = base + ".root"

        if info["sumw"] is None:
            print(f"[WARN] sumw missing in {txt_path} -> skip this entry")
            continue

        sumw_map[root_name] = float(info["sumw"])
        meta_map[root_name] = info

    return sumw_map, meta_map

# ============================================================
# 7) helper: 히스토 이름 추출 / 변환
# ============================================================
...
# ============================================================
# 8) sumw_txt 미리 로드 (전체 region 공통)
# ============================================================
sumw_map, meta_map = load_sumw_maps(SUMW_TXT_DIR)
print(f"[INFO] loaded sumw_txt: {len(sumw_map)} entries from {SUMW_TXT_DIR}")
...
for region in regions:
    ...
    mc_dir_1 = os.path.join(MC_BASE, region)
    mc_dir_2 = os.path.join(DATA_BASE, region)
    ...
    if not used_eras:
        ...
        print(f"[WARN] {region}: data 파일 발견={len(existing)}개, era 추출 실패 → 스킵")
        continue
    ...
    for hname in hist_names:
        ...
        for p in mc_paths:
            ...
            if base == "QCD_ABCD.root":
                ...
                tmp.SetDirectory(0)
                tmp.Sumw2()
                tmp.Scale(1.0)  # QCD template은 이미 yield이므로 1
                ...
                # ✅ 이제 분모(sumw)는 sumw_txt에서만 가져온다
                sumw = sumw_map.get(base, None)
                if sumw is None or sumw <= 0:
                    print(f"   [SKIP] sumw_txt missing/invalid for: {base}")
                    f.Close()
                    continue
                ...
                nbad = meta.get("nBad", 0)
                if nbad and int(nbad) > 0:
                    # 강하게 스킵하고 싶으면 아래 print 대신 continue로 바꾸면 됨
                    print(f"   [WARN] {base}: nBad={nbad} in sumw_txt (sumw may be partial depending on how you built it)")
